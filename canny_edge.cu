...
__constant__ float kernal_constant[15];
/******************************************************************************
 ...
 *******************************************************************************/
 ...
 __global__ void derrivative_delta(short * delta_x,short * delta_y,short int* smoothedim,int rows,int cols){
   ...
   if (c == 0)
   delta_x[posx] = smoothedim[posx + 1] - smoothedim[posx];
   else if (c == (cols-1))
   delta_x[posx] = smoothedim[posx] - smoothedim[posx - 1];
   else
   delta_x[posx] = smoothedim[posx + 1] - smoothedim[posx - 1];

   // for deltay
   int posy = r * cols + c;
   if (r == 0)
   delta_y[posy] = smoothedim[posy + cols] - smoothedim[posy];
   else if( r == (rows-1))
   delta_y[posy] = smoothedim[posy] - smoothedim[posy - cols];
   else
   delta_y[posy] = smoothedim[posy + cols] - smoothedim[posy - cols];
                     ...
                     short int **delta_x, short int **delta_y)
{
   ...
   *delta_y = (short *)malloc(rows * cols * sizeof(short));
   ...
   cudaMalloc(&d_delta_x,rows * cols * sizeof(short));
   cudaMalloc(&d_delta_y,rows * cols * sizeof(short));
   cudaMalloc(&d_smoothedim,rows * cols * sizeof(short int));
   ...
   int TPB = 512;
   int BlockSize = ((rows*cols+TPB-1)/TPB);

   // Launch Kernal
   derrivative_delta<<<BlockSize,TPB>>>(d_delta_x,d_delta_y, d_smoothedim, rows , cols);
   cudaDeviceSynchronize();
   cudaMemcpy(*delta_x,d_delta_x,rows * cols * sizeof(short),cudaMemcpyDeviceToHost);
   cudaMemcpy(*delta_y,d_delta_y,rows * cols * sizeof(short),cudaMemcpyDeviceToHost);
 ...
 *******************************************************************************/
 ...
 __global__ void gaussian_tempim(float* tempim,unsigned char* image,int center,int rows,int cols)
 {
   ...
   for (int cc = (-center); cc <= center; cc++)
   {
      if (((c + cc) >= 0) && ((c + cc) < cols))
      {
      dot += (float)image[r * cols + (c + cc)] * kernal_constant[center + cc];
      sum += kernal_constant[center + cc];
      }
   }
...
__global__ void gaussian_smoothedim(short int *smoothedim, float *tempim, int center ,int rows, int cols) 
   {
   int index = blockIdx.x*blockDim.x+threadIdx.x;
   int r = index / cols;
   int c = index % cols;
   ...
   for (int rr = (-center); rr <= center; rr++)
   {
    if (((r + rr) >= 0) && ((r + rr) < rows))
      {
      dot += tempim[(r + rr) * cols + c] * kernal_constant[center + rr];
      sum += kernal_constant[center + rr];
      }
   }
...
cudaMalloc(&d_tempim,rows * cols * sizeof(float));
cudaMalloc(&d_image,rows*cols*sizeof(unsigned char));

//copy data from host to device
// This is constant memory as this not changes throughpput the filtering
cudaMemcpyToSymbol(kernal_constant, kernel, sizeof(float) * 15);
...
//  Launch Kernal
gaussian_tempim<<<BlockSize,TPB>>>(d_tempim,d_image,center,rows,cols);
cudaDeviceSynchronize();
cudaMemcpy(tempim,d_tempim,(rows * cols * sizeof(float)),cudaMemcpyDeviceToHost);

/****************************************************************************
* Blur in the y - direction.
****************************************************************************/
cudaMalloc(&d_smoothedim,rows * cols * sizeof(short int));
cudaMemcpy(d_tempim,tempim,(rows * cols * sizeof(float)),cudaMemcpyHostToDevice);
gaussian_smoothedim<<<BlockSize,TPB>>>(d_smoothedim,d_tempim, center,rows,cols);
cudaDeviceSynchronize();
cudaMemcpy(*smoothedim,d_smoothedim,rows * cols * sizeof(short int),cudaMemcpyDeviceToHost);
...
void make_gaussian_kernel(float sigma, float **kernel, int *windowsize)
{
   ...
   *windowsize = 1 + 2 * ceil(2.5 * sigma);
   ...
   printf("Window Size in make_gaussian_kernel %d\n", *windowsize);
